...
const MenuView = ({ selectedCategory, onSelectCategory, items, onAddToCart }) => (
  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div className="scrollbar-hide mb-10 flex gap-4 overflow-x-auto border-b border-white/5 pb-4">
      {MENU_CATEGORIES.map((category) => (
        <button
          key={category}
          onClick={() => onSelectCategory(category)}
          className={`whitespace-nowrap rounded-full border px-6 py-2 text-xs uppercase tracking-[0.2em] transition-all ${
    ...
    <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
        <div
          ...
          className="glass-panel hover-glow group relative cursor-pointer overflow-hidden rounded-[2rem] p-5 transition-all duration-300"
        >
          ...
          <div className="mb-2 flex items-start justify-between gap-3">
            <h3 className="font-serif text-xl tracking-tight text-white">{item.name}</h3>
            <span className="font-mono text-lg font-bold text-[#E5C07B]">${item.price}</span>
          </div>
